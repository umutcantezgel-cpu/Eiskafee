...
test.describe("SyrupCursor", () => {
  test("DOM Injection: Verify the custom cursor container is present in the DOM upon page load without triggering Next.js hydration errors", async ({
    page,
  }) => {
    const logs: string[] = [];
    page.on("console", (msg) => {
  ...
  test("Native Cursor Hidden: Verify that the <body> or main container has cursor: none applied", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("Movement Tracking: Move the mouse via Playwright and assert the custom cursor element updates to match coordinates", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("Hover State Interaction: Hover over a clickable element and verify the cursor element receives an active/hover state", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    const hoverTransform = await cursor.evaluate(
      (el) => window.getComputedStyle(el).transform,
    );
  ...
  test("Cleanup Validation: Navigate between pages rapidly and check the console to ensure no requestAnimationFrame leaks", async ({
    page,
  }) => {
    const logs: string[] = [];
    page.on("console", (msg) => {
    ...
    expect(
      logs.filter(
        (log) =>
          ...
          log.toLowerCase().includes("unmounted"),
      ),
    ).toHaveLength(0);
  });
});
