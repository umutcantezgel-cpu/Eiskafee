...
- Name: tier1_static_navigation.spec.ts >> Tier 1: Core Navigation & Static Content >> Global UI: Navigation links functionality
- Location: tests/e2e/tier1_static_navigation.spec.ts:98:3
...
Locator: locator('header a[href^="/"]').filter({ hasText: /menu/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('header a[href^="/"]').filter({ hasText: /menu/i }).first()
