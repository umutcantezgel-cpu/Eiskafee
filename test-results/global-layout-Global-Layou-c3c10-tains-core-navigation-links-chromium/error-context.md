...
- Name: global-layout.spec.ts >> Global Layout >> Header Rendering: Verify the header is visible and contains core navigation links
- Location: tests/e2e/global-layout.spec.ts:4:7
...
Locator: locator('header').getByRole('link', { name: 'Home' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('header').getByRole('link', { name: 'Home' })
