...
export default function AdminUsersPage() {
  ...
  const loadUserDetails = async (u: UserData) => {
    ...
    setUserOrders(
      orderSnap.docs
        ...
        .filter((o: any) => o.userId === u.id || o.customerEmail === u.email),
    );
  ...
  const handleRoleChange = async (newRole: "admin" | "customer") => {
    ...
    if (
      confirm(
        `Wirklich die Rolle von ${selectedUser.name || selectedUser.email} auf ${newRole.toUpperCase()} ändern?`,
      )
    ) {
      try {
  ...
  const handleAddCoins = async () => {
    ...
    try {
      ...
      await setDoc(
        doc(db, "loyalty_points", selectedUser.id),
        { balance: newBalance },
        { merge: true },
      );
      ...
      <div className="flex-1">
        ...
        {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {filteredUsers.map((u) => (
                  <tr
                    key={u.id}
                    className="border-b border-peach/50 hover:bg-peach/10 transition-colors"
                  >
                    <td className="py-3 px-6 font-bold text-charcoal">
                      {u.name || "-"}
                    </td>
                    <td className="py-3 px-6 text-charcoal/80 text-sm">
                      {u.email || "-"}
                    </td>
                    <td className="py-3 px-6 text-center">
                      <span
                        className={`px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${u.role === "admin" ? "bg-terracotta text-cream" : "bg-mint/20 text-mint"}`}
                      >
                        {u.role}
                      </span>
                    </td>
                    <td className="py-3 px-6 text-right">
                      <button
                        ...
                        className="p-2 text-terracotta bg-peach/30 rounded-full hover:bg-peach/80 transition-colors"
                      >
          ...
          <div className="space-y-4">
            {messages.length === 0 ? (
              <div className="text-center p-8 bg-cream rounded-3xl border border-peach text-charcoal/60 font-bold">
              ...
              messages.map((m) => (
                <div
                  key={m.id}
                  className="bg-cream rounded-3xl p-6 shadow-sm border border-peach"
                >
                  ...
                  <p className="mt-4 text-charcoal/80 text-sm leading-relaxed whitespace-pre-wrap">
                    {m.message}
                  </p>
                </div>
              ))
        ...
        <div className="w-96 bg-cream rounded-3xl border border-peach shadow-sm flex flex-col h-[calc(100vh-8rem)] sticky top-8">
          <div className="p-6 border-b border-peach flex justify-between items-start">
            ...
            <button
              ...
              className="p-1 bg-peach/30 text-terracotta rounded-full"
            >
              <X size={18} />
            </button>
          </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setCoinAmountToAdd(parseInt(e.target.value) || 0)
                  }
                  placeholder="Betrag"
                ...
                <button
                  ...
                  className="bg-terracotta text-cream px-4 rounded-xl font-bold text-sm"
                >
                  Gutschreiben
                </button>
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {userOrders.slice(0, 5).map((o) => (
                  <div
                    key={o.id}
                    className="bg-sand p-3 rounded-xl flex justify-between items-center"
                  >
                    <div>
                      <div className="font-bold text-charcoal text-sm">
                        {o.orderNumber || "Online Order"}
                      </div>
                      <div className="text-[10px] text-charcoal/60 font-bold">
                ...
                {userOrders.length === 0 && (
                  <div className="text-sm text-charcoal/50 font-bold italic">
                    Keine Bestellungen
                  </div>
                )}
              </div>
