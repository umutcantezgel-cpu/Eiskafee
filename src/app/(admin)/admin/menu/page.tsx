    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      {loading ? (
        <div className="animate-pulse flex flex-col gap-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-20 bg-cream rounded-2xl" />
          ))}
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-peach">
                <th className="py-4 px-6 font-heading text-terracotta">
                  Produkt
                </th>
                <th className="py-4 px-6 font-heading text-terracotta hidden md:table-cell">
                  Kategorie
                </th>
                <th className="py-4 px-6 font-heading text-terracotta">
                  Preis
                </th>
                <th className="py-4 px-6 font-heading text-terracotta text-center">
                  Verfügbar
                </th>
                {/* <th className="py-4 px-6 font-heading text-terracotta text-right">Aktion</th> */}
              </tr>
            </thead>
            <tbody>
              {filteredItems.map((item) => (
                ...
                >
                  <td className="py-4 px-6">
                    ...
                    />
                    <div className="text-xs text-charcoal/60 line-clamp-1 max-w-xs">
                      {item.desc}
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-6 text-center">
                    ...
                    >
                      <span
                        className={`inline-block h-5 w-5 transform rounded-full bg-cream transition-transform shadow-sm ${item.available ? "translate-x-6" : "translate-x-1"}`}
                      />
                    </button>
                  </td>
                  {/* <td className="py-4 px-6 text-right">
                    <button 
                      ...
                      title="Bearbeiten deaktiviert: Die Speisekarte wird zentral aus dem Code geladen."
                    >
