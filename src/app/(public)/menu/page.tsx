...
export const metadata = buildMetadata({
  title: "Speisekarte Wetzlar · Bubble Waffles, Crêpes & Eis | Hey Fede",
  description:
    "Entdecke die komplette Speisekarte von Hey Fede in Wetzlar: Frisch gebackene Bubble Waffles, Crêpes, Pancakes, Eisbecher, Special Shakes & Fede Boxen.",
  path: "/menu",
  keywords: [
    "Speisekarte Hey Fede Wetzlar",
...
const menuCategories = Object.entries(HF_DATA.menu).map(([key, cat]) => ({
  ...
  items: (cat as { items: { name: string; price: string; desc: string }[] })
    .items,
}));
...
export default function MenuPage() {
  return (
    <>
      ...
      <SEOContentBlock
        visible={true}
        ariaLabel="Wissenswertes rund um unsere Speisekarte"
      >
        <MenuSEO />
      </SEOContentBlock>
      <SchemaScripts
        schema={[
          buildMenuSchema(menuCategories),
          buildBreadcrumbSchema([{ name: "Speisekarte", path: "/menu" }]),
        ]}
      />
    </>
  );
