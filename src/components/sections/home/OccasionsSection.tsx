...
export function OccasionsSection() {
  const occasions = [
    ...
    {
      icon: <GraduationCap className="w-8 h-8 text-terracotta" />,
    ...
    {
      icon: <ShoppingBag className="w-8 h-8 text-terracotta" />,
      ...
      <FadeUp delay={0.1} className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">
          {occasions.map((occ, i) => (
            <div
              key={i}
              className="hf-hover-lift bg-cream rounded-3xl p-8 border border-peach/50 transition-colors shadow-sm"
            >
              <div className="bg-cream w-16 h-16 rounded-full flex items-center justify-center mb-6 shadow-sm border border-peach/30">
                {occ.icon}
              </div>
              <h3 className="font-heading text-2xl text-brown mb-4">
                {occ.title}
              </h3>
              <p className="font-body text-brown-mid leading-relaxed text-[0.95rem]">
                {occ.description}
              </p>
            </div>
          ))}
