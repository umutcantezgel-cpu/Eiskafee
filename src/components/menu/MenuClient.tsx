      ...
      <div className="group bg-white rounded-[18px] p-[16px_18px] flex gap-3.5 items-center transition-all duration-250 hover:-translate-y-0.5 hover:shadow-[0_6px_22px_rgba(204,98,76,0.13)] shadow-[0_2px_10px_rgba(45,31,25,0.05)] hover:bg-[#fef8f5]">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start gap-2 mb-1">
            ...
            <span className="font-calistoga text-[1.05rem] text-[#b34832] shrink-0">
              {item.price}
            </span>
          </div>
          {item.desc && (
        ...
        <div className="flex flex-col items-end justify-between h-full py-1">
          {item.tag && (
            <div className="bg-[#eedfcc] text-[#b34832] font-nunito font-black text-[0.64rem] px-[11px] py-1 rounded-full shrink-0 tracking-[0.5px]">
              {item.tag}
            </div>
          )}
          <button
            ...
            className="mt-auto w-8 h-8 rounded-full bg-[#b34832] flex items-center justify-center text-white hover:bg-[#b5523d] transition-colors shadow-sm shrink-0"
          >
...
const BoxCard = React.memo(
  ({ item, onAdd }: { item: any; onAdd: (item: any) => void }) => {
    return (
      <div className="group bg-white border-2 border-dashed border-[#b34832] rounded-xl p-[20px_22px] relative transition-all duration-[280ms] hover:-translate-y-1 hover:shadow-[0_8px_28px_rgba(204,98,76,0.18)] shadow-[0_2px_12px_rgba(45,31,25,0.06)] hover:bg-[#fef8f5] mx-3">
        {/* Perforation holes */}
        ...
        {item.tag && (
          <div className="absolute -top-[11px] right-[18px] bg-[#b34832] text-white font-nunito font-black text-[0.64rem] px-3.5 py-1 rounded-full tracking-[1px] uppercase">
            {item.tag}
          </div>
        )}
        ...
        <div className="flex gap-3.5 items-start">
          ...
          <div className="flex-1">
            <h3 className="font-calistoga text-[1.08rem] text-[#2d1f19] mb-1.5">
              {item.name}
            </h3>
            <p className="font-nunito text-[0.78rem] text-[#7a5a52] leading-[1.55] mb-2">
              {item.desc}
            </p>
            <div className="flex items-center justify-between mt-3">
              <span className="font-calistoga text-[1.3rem] text-[#b34832]">
                {item.price}
              </span>
              <button
                onClick={(e) => {
                ...
                className="px-4 py-2 rounded-full bg-[#b34832] flex items-center justify-center text-white font-nunito font-bold text-xs hover:bg-[#b5523d] transition-colors shadow-[0_4px_12px_rgba(204,98,76,0.25)]"
              >
                Hinzufügen
              </button>
            </div>
...
export function MenuClient({
    ...
    const loadFromFirebase = async () => {
      try {
        if (
          ...
          db.app.options.apiKey === "dummy" ||
          db.app.options.apiKey === "DEIN_API_KEY"
        )
          return;

        const catSnap = await getDocs(collection(db, "categories"));
        if (!catSnap.empty) {
          const loadedCats = catSnap.docs.map((d) => ({
            id: d.id,
            ...d.data(),
          }));
          setCategories(
        ...
        if (!menuSnap.empty) {
          ...
          menuSnap.docs.forEach((doc) => {
            const data = doc.data();
            if (!newMenu[data.categoryId])
              newMenu[data.categoryId] = { items: [] };
  ...
  useEffect(() => {
    const fn = () => {
      if (tabsRef.current)
        setSticky(tabsRef.current.getBoundingClientRect().top <= 68);
    };
    ...
    <div className="min-h-screen bg-[#f5efe8] animate-fade-in">
      ...
      <div className="bg-[#E4C0A8] pt-[100px] px-6 pb-[72px] text-center relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 leading-none">
          ...
          >
            <path
              d="M0,22 C320,46 640,0 960,22 C1120,34 1300,14 1440,22 L1440,44 L0,44 Z"
              fill="#f5efe8"
            />
          </svg>
        </div>
      ...
      >
        <div className="max-w-[1200px] mx-auto px-6">
          <div
            ref={tabsRowRef}
            className="flex gap-2 overflow-x-auto py-[13px] scroll-hide snap-x snap-mandatory"
          >
            {categories.map((c: any) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 font-nunito font-extrabold text-[0.8rem] px-4 py-[9px] rounded-full border-none cursor-pointer transition-all duration-[220ms] whitespace-nowrap shrink-0 snap-start ${isActive ? "bg-[#b34832] text-white shadow-[0_3px_12px_rgba(204,98,76,0.3)]" : "bg-[#eedfcc] text-[#5c3d35]"}`}
                >
      ...
      <div className="max-w-[880px] mx-auto pt-11 px-6 pb-20">
        ...
        {/* Category header */}
        <div className="mb-7">
          <h2 className="font-calistoga text-[clamp(1.5rem,3vw,2.1rem)] text-[#2d1f19] mb-1.5">
            {cat?.label}
          </h2>
          {data.desc && (
            <p className="font-nunito text-[0.88rem] text-[#9a7060]">
              {data.desc}
            </p>
          )}
          <div className="w-11 h-[3px] bg-[#b34832] rounded-sm mt-3" />
        </div>
        ...
        <div className="flex flex-col gap-3">
          {activeId === "bubble" ? (
            ...
            <div className="text-center py-10 bg-white/40 rounded-[18px] border border-dashed border-[#eedfcc]">
              <p className="font-nunito text-charcoal/60">
        ...
        {data.note && (
          <div className="bg-[#eedfcc] rounded-[14px] p-[14px_18px] mt-4.5 flex gap-2 items-start">
            <div className="w-[7px] h-[7px] bg-[#b34832] rounded-full mt-1 shrink-0" />
            <p className="font-nunito text-[0.84rem] text-[#5c3d35] italic leading-[1.55]">
              {data.note}
            </p>
          </div>
        )}
        ...
        {(activeId === "kuchen" || activeId === "cookies") && (
          <div className="mt-6 bg-[#eedfcc] rounded-[18px] p-[20px_22px] flex gap-4 items-center">
            ...
            <p className="font-nunito text-[0.86rem] text-[#5c3d35] italic leading-[1.6]">
              Psst... frischen, leckeren Kuchen findest du in unserer Vitrine
              vor Ort in der {BUSINESS.street} in {BUSINESS.city}!
            </p>
          </div>
        )}
      </div>
