...
export function FoodIcon({ icon = 'Sparkles', label, size = 'md', shape = 'rounded-2xl', className = '' }: { icon?: string, label?: string, size?: string, shape?: string, className?: string }) {
  ...
  return (
    <div className={`bg-[#E4C0A8] flex flex-col items-center justify-center gap-2 overflow-hidden flex-shrink-0 ${s.cls} ${radius} ${className}`}>
      <LIcon size={s.px} strokeWidth={1.4} color="#CC624C" style={{ opacity: 0.65 }} />
      {label && (
        <span style={{ fontFamily: 'monospace', fontSize: 9, color: '#CC624C', opacity: 0.8, textAlign: 'center', lineHeight: 1.4, padding: '0 8px', whiteSpace: 'pre-line' }}>
          {label.replace(/\\n/g, '\\n')}
        </span>
      )}
    </div>
