  ...
  const innerContent = (
    <>
      ...
      >
        {/* Decorative circles */}
        <div
          className="absolute top-3 left-3 w-16 h-16 rounded-full opacity-20"
          style={{ background: "#b34832" }}
        />
        <div
          className="absolute bottom-2 right-4 w-10 h-10 rounded-full opacity-15"
          style={{ background: "#b34832" }}
        />
        <LIcon
          size={64}
      ...
      <motion.div style={{ transform: "translateZ(30px)" }}>
        <h3 className="font-calistoga text-[1.1rem] text-[#2d1f19] leading-tight mb-1">
          {item.name}
        </h3>
        <p className="font-nunito text-sm text-[#5c3d35] leading-relaxed mb-4">
          {item.desc}
        </p>

        <div className="flex justify-between items-center mt-auto">
          <span className="font-calistoga text-xl text-[#b34832]">
            {item.price}
          </span>
          <button
