...
export default function PhysicsPlayground({ size = 360 }: { size?: number }) {
  ...
  const items = useMemo(() => {
    const arr = [];
    for (let i = 0; i < 14; i++) arr.push({ ...DESSERT_ITEMS[i % DESSERT_ITEMS.length], id: i });
    return arr;
  }, []);
  ...
  useEffect(() => {
    ...
    const engine = Mt.Engine.create({ gravity: { x: 0, y: 0.85, scale: 0.001 } });
    ...
    items.forEach((item, i) => {
      ...
      const body = item.type === 'square'
        ...
        : Mt.Bodies.circle(x, y, item.size/2, { restitution: 0.78, friction: 0.05, density: 0.0015 });
    ...
    Mt.Events.on(engine, 'afterUpdate', () => {
      ...
      bodyRefs.current.forEach(body => {
        ...
        if (body.position.y > ch + 300 || body.position.y < -600 || body.position.x < -300 || body.position.x > cw + 300) {
          // Respawn at the top
          Mt.Body.setPosition(body, { x: 60 + Math.random() * (cw - 120), y: -60 - Math.random() * 50 });
    ...
    const ro = new ResizeObserver(([entry]) => {
      const r = entry.contentRect; w = r.width; h = r.height;
      if (w === 0 || h === 0) return;
  ...
  const handleDoubleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (clickedBodyIndex !== -1) {
      ...
      dom.style.transition = 'all 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
      ...
      const confettiColors = ['#CC624C', '#E4C0A8', '#eedfcc', '#5c3d35', '#f5efe8'];
      ...
      setTimeout(() => {
        ...
        Matter.Body.setPosition(body, { x: 60 + Math.random() * (cw - 120), y: -80 - Math.random() * 50 });
        Matter.Body.setVelocity(body, { x: 0, y: 0 });
    ...
    >
      <div 
        style={{
          ...
          color: 'rgb(204, 98, 76)', opacity: 0.6, pointerEvents: 'none',
      ...
      {items.map((item, i) => {
        ...
        return (
          ...
          >
            <IconComponent size={item.size * 0.45} color="#fff" strokeWidth={2} />
          </div>
        );
      })}
    </div>
